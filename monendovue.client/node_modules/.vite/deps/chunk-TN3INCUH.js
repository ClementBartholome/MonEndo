...
var isEqual = (a, b, visited = /* @__PURE__ */ new Set()) => {
  if (Array.isArray(a)) {
...
function countUnique(array2, accessor = (d) => d) {
  return new Set(array2.map((d) => accessor(d))).size;
}
...
function getStackedData(data, baseline, acs, prevNegative) {
  ...
  stackedData.filter((s) => s.negative).forEach((s, i, arr) => {
...
function getNearest(data, value, accessor) {
  ...
  const index2 = xBisector(values, value, 1, data.length - 1);
  return value - values[index2 - 1] > values[index2] - value ? data[index2] : data[index2 - 1];
}
function filterDataByRange(data, range2, accessor) {
...
//# sourceMappingURL=chunk-TN3INCUH.js.map
